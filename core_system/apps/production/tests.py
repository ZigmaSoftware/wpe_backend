...
class ProductionLineConnectApiTests(APITestCase):
    def setUp(self):
        ...
        self.item = Item.objects.create(
            ...
            group="Granules",
            sub_group="WPE",
            item_name=f"HSN - GL-Test {self.unique_suffix}",
            unit="KG",
        )
        self.machine_1 = ProductionMachine.objects.create(
            machine_code=f"MCH-LC-{self.unique_suffix[:4].upper()}-1",
            name="machine 1",
            machine_type=ProductionMachine.MachineType.GRANULATION,
            applicable_stages="GL,PR",
            serial_no=f"SER-{self.unique_suffix[:4].upper()}-1",
            is_active=True,
        )
        self.machine_2 = ProductionMachine.objects.create(
            machine_code=f"MCH-LC-{self.unique_suffix[:4].upper()}-2",
            name="Machine 2",
            machine_type=ProductionMachine.MachineType.GRANULATION,
            applicable_stages="GL,PR",
            serial_no=f"SER-{self.unique_suffix[:4].upper()}-2",
            is_active=True,
        )
        self.line_1 = ProductionLineMaster.objects.create(
            name="Line 1",
            machine=self.machine_1,
            status=ProductionLineMaster.LineStatus.FREE,
            is_active=True,
        )
        self.line_2 = ProductionLineMaster.objects.create(
            name="Line 2",
    ...
    def _create_order(self, production_id: str, stage: str, production_type: str):
        return ProductionOrder.objects.create(
            production_id=production_id,
            production_type=production_type,
            production_date=date.today(),
            extra_form_data={"stage": stage},
        )

    def _create_connection_to_line_stock(
        self,
        *,
        production_id: str,
        stage: str,
        ...
        order = self._create_order(production_id, stage, production_type)
        batch = ProductionBatch.objects.create(
            batch_no=batch_no,
        ...
        inventory_row = ProductionInventoryTransaction.objects.create(
            movement_key=f"line-connect:{scan_code}",
            stage=ProductionInventoryTransaction.Stage.CONNECTION_TO_LINE,
            batch_code=batch_no,
            production_order=order,
            ...
            item_code=self.item.item_code,
            item_name=self.item.item_name,
            inward_qty=Decimal(weight),
            outward_qty=Decimal("0.000"),
            balance_qty=Decimal(weight),
            uom="KG",
        ...
        return order, batch, output_capture, inventory_row

    def test_lookup_returns_connection_to_line_bag_details(self):
        _source_order, _batch, output_capture, inventory_row = self._create_connection_to_line_stock(
            production_id=f"GL-LC-{self.unique_suffix.upper()}",
            stage="GL",
            production_type="WPE Granulated Blend Production",
            batch_no=f"BATCHGL-LC-{self.unique_suffix[:4].upper()}-01",
            baglot="BAG001",
            scan_code=f"GL02GL{format_scancode_time(timezone.now())}01",
            reference_no="BATCHGL-0101",
            weight="4.200",
        )

        response = self.client.get(
            "/api/production/line-connections/lookup/",
            {"scan": output_capture.scancode_id},
        )

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        payload = response.data["data"]
        self.assertEqual(payload["inventory_transaction_id"], inventory_row.id)
        self.assertEqual(payload["baglot"], "BAG001")
        self.assertEqual(payload["scancode"], output_capture.scancode_id)
        self.assertEqual(payload["available_weight"], "4.200")
        self.assertEqual(payload["status"], "AVAILABLE")
        self.assertEqual(payload["item"]["item_code"], self.item.item_code)

    def test_connect_endpoint_blocks_duplicate_bag_and_occupied_line(self):
        _source_order, _batch, output_capture, _inventory_row = self._create_connection_to_line_stock(
            production_id=f"GL-CONN-{self.unique_suffix.upper()}",
            stage="GL",
            production_type="WPE Granulated Blend Production",
            batch_no=f"BATCHGL-LC-{self.unique_suffix[:4].upper()}-11",
            baglot="BAG001",
            scan_code=f"GL02GL{format_scancode_time(timezone.now())}11",
        ...
        connect_response = self.client.post(
            ...
            {
                "scan_code": output_capture.scancode_id,
                "production_line_id": self.line_1.id,
            },
            format="json",
        )

        self.assertEqual(connect_response.status_code, status.HTTP_200_OK)
        self.assertEqual(ProductionLineConnection.objects.filter(status="ON").count(), 1)

        duplicate_bag_response = self.client.post(
            "/api/production/line-connections/connect/",
            {
                "scan_code": output_capture.scancode_id,
                "production_line_id": self.line_2.id,
            },
            format="json",
        )

        self.assertEqual(duplicate_bag_response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("already connected", duplicate_bag_response.data["message"])

        _source_order_2, _batch_2, output_capture_2, _inventory_row_2 = self._create_connection_to_line_stock(
            production_id=f"GL-CONN2-{self.unique_suffix.upper()}",
            stage="GL",
            production_type="WPE Granulated Blend Production",
            batch_no=f"BATCHGL-LC-{self.unique_suffix[:4].upper()}-12",
            baglot="BAG002",
            scan_code=f"GL02GL{format_scancode_time(timezone.now())}12",
            reference_no="REF-002",
            weight="5.000",
        )

        occupied_line_response = self.client.post(
            "/api/production/line-connections/connect/",
            {
                "scan_code": output_capture_2.scancode_id,
                "production_line_id": self.line_1.id,
            },
            format="json",
        )

        self.assertEqual(occupied_line_response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("already connected with another bag", occupied_line_response.data["message"])

    def test_pr_consumption_auto_disconnects_active_line_connection_when_bag_is_empty(self):
        source_order, _source_batch, output_capture, inventory_row = self._create_connection_to_line_stock(
            production_id=f"GL-AUTO-{self.unique_suffix.upper()}",
            stage="GL",
            production_type="WPE Granulated Blend Production",
            batch_no=f"BATCHGL-LC-{self.unique_suffix[:4].upper()}-21",
            baglot="BAG003",
            scan_code=f"GL02GL{format_scancode_time(timezone.now())}21",
            reference_no="REF-003",
            weight="4.200",
        )

        self.line_1.status = ProductionLineMaster.LineStatus.RUNNING
        self.line_1.save(update_fields=["status", "updated_at"])

        active_connection = ProductionLineConnection.objects.create(
            source_inventory_transaction=inventory_row,
            source_production_order=source_order,
            production_line=self.line_1,
            machine=self.machine_1,
            item=self.item,
            item_code=self.item.item_code,
            item_name=self.item.item_name,
            baglot="BAG003",
            scancode=output_capture.scancode_id,
            reference_no=inventory_row.reference_no or "",
            weight=Decimal("4.200"),
            line_name=self.line_1.name,
            machine_name=self.machine_1.name,
            connected_by=self.user,
        )

        pr_order = self._create_order(
            f"PR-LC-{self.unique_suffix.upper()}",
            "PR",
            "WPE Production Line",
        )
        pr_batch = ProductionBatch.objects.create(
            batch_no=f"BATCHPR-LC-{self.unique_suffix[:4].upper()}-01",
            workflow_batch_no=f"BATCHPR-LC-{self.unique_suffix[:4].upper()}-01",
            production_order=pr_order,
            stage=ProductionBatch.Stage.PR,
        )
        pr_output_capture = ProductionOutputCapture.objects.create(
            production_order=pr_order,
            source_batch=pr_batch,
            sequence=1,
            scancode_id=f"PR02PR{format_scancode_time(timezone.now())}01",
            recipe_no="",
            quantity_kg="4.200",
            weight_kg="4.200",
            binlot="PR-BAG001",
        ...
        inventory_row.refresh_from_db()
        active_connection.refresh_from_db()
        self.line_1.refresh_from_db()

        self.assertEqual(str(inventory_row.balance_qty), "0.000")
        self.assertEqual(active_connection.status, ProductionLineConnection.ConnectionStatus.OFF)
        self.assertEqual(self.line_1.status, ProductionLineMaster.LineStatus.FREE)
